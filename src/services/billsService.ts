...
export async function updateManyBills(
	data: BillsColumns[],
	paymentMethod: { _paymentType: string; _paymentDate: Date }
) {
	const jwt = cookies().get("Authorization")?.value!;
	if (!jwt) {
		throw new Error(
...
export async function updateOneBill(data: BillsColumns) {
	...
	let response = await fetch(
		`${process.env.BACKEND_URL}/bills/editing/${data._id._id}`,
		{
			method: "PUT",
			headers: {
				Authorization: jwt,
...
export async function deleteBill(data: BillsColumns) {
	...
	await fetch(process.env.BACKEND_URL + "/bills/" + data._id._id, {
		method: "DELETE",
		headers: {
			Authorization: jwt,
			"Content-Type": "application/json"
