		...
		<main className="flex h-screen w-screen items-center justify-center bg-cover p-5 md:p-10">
			{isConfirmed === null ? (
				...
				<section className="flex h-full w-full flex-row items-center justify-around rounded-[30px] border border-primary-500 bg-gradiente-vidro p-9 md:w-4/5">
					<section className="w-full text-primary-950 md:w-2/5">
						{isConfirmed ? (
							<>
								<h2 className="text-3xl">E-mail confirmado</h2>
								<p className="mb-4 mt-8 text-xl">
									E-mail confirmado com sucesso, agora você
									pode fazer login.
								</p>
							</>
						) : (
							<>
								...
								<p className="mb-4 mt-8 text-xl">
									Problemas ao confirmar seu email. Verifique
									e entre em contato com a equipe de suporte.
								</p>
							</>
						)}
					</section>
					{isConfirmed === true && (
						<section className="hidden lg:block lg:w-2/5">
							<Image
								src={confirmImage}
								alt="Moça recebendo carta de um passarinho"
							/>
						</section>
					)}
				</section>
